...
\begin{frame}{Harmonic Closeness Centrality}
    ...
    \begin{columns}[T]
        ...
        \vspace{0.2cm}
        \textbf{Formula:}
        $C_H(v) = \sum_{u \neq v} \frac{1}{d(v, u)}$ \quad ($d$ = shortest path)
        
        \vspace{0.2cm}
        \textbf{Interpretation:}
        \begin{itemize}\setlength{\itemsep}{1pt}
            \item Higher score = better reachability
            \item Central nodes in network
        \end{itemize}
        ...
        \begin{itemize}\setlength{\itemsep}{1pt}
            ...
            \item Robust to disconnected components
        \end{itemize}
    \end{columns}
\end{frame}
...
\begin{frame}{Betweenness Centrality (Brandes Algorithm)}
    ...
    \begin{columns}[T]
        ...
        \vspace{0.1cm}
        {\footnotesize $\sigma_{st}$ = \# paths $s \to t$; $\sigma_{st}(v)$ = paths through $v$}
        
        \column{0.5\textwidth}
        \textbf{Complexity:}
        \begin{block}{Time: $\Theta(nm)$ unweighted}
        \end{block}
        \vspace{-0.1cm}
        \begin{block}{Space: $\Theta(n + m)$}
        \end{block}
        ...
        \begin{itemize}\setlength{\itemsep}{1pt}
            \item Avoids recomputing all paths
            \item Dependency accumulation
            \item Single BFS per source
            \item Backward accumulation
        \end{itemize}
    \end{columns}
\end{frame}
...
\begin{frame}{PageRank}
    ...
    \begin{columns}[T]
        ...
        \begin{itemize}\setlength{\itemsep}{1pt}
            ...
            \item Important if important nodes link to it
            \item Iterative power-method updates
            \item Damping factor for convergence
        \end{itemize}
        ...
        \vspace{0.1cm}
        {\footnotesize where $d \approx 0.85$ is damping factor}
        ...
        \begin{itemize}\setlength{\itemsep}{1pt}
            ...
            \item Converges in few iterations
...
\begin{frame}{Hybrid Friend Recommender}
    ...
    \begin{block}{Final Score}
        $Score(u, v) = w_1 \cdot J(u,v) + w_2 \cdot AA(u,v) + w_3 \cdot T(u,v)$ \quad (Top-$k$ become recommendations)
    \end{block}
\end{frame}
...
\begin{frame}{Recommendation: Key Findings}
    ...
    \begin{block}{Performance Summary}
        \begin{itemize}\setlength{\itemsep}{1pt}
            \item Single-user: $<$ 10ms at $n=1000$ → suitable for interactive use
            \item 40\% hit rate @ $k=10$ → 4/10 users get useful recommendations
            \item 10× better than random, 27\% better than common neighbors
        \end{itemize}
    \end{block}
    \vspace{-0.1cm}
    \begin{block}{Multi-Signal Benefits}
        \begin{itemize}\setlength{\itemsep}{1pt}
            ...
            \item Tags: content-based, helps with cold-start; provides noise robustness
        \end{itemize}
    \end{block}
    \vspace{-0.1cm}
...
\begin{frame}[t]
    ...
    \begin{block}{Definition --- Measures quality of a community partition}
        ...
        \vspace{-0.3cm}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $A_{ij}$: adjacency matrix; $k_i, k_j$: degrees; $m$: total edges
            \item $\delta(c_i, c_j) = 1$ if $i, j$ in same community, 0 otherwise
        \end{itemize}
    \end{block}
    \vspace{-0.1cm}
    \begin{columns}[T]
        ...
        \begin{itemize}\setlength{\itemsep}{1pt}
            ...
            \item Actual vs expected edges (null model)
        \end{itemize}
...
\begin{frame}[t]
    ...
    \vspace{0.1cm}
    \footnotesize
    \begin{block}{Five Comprehensive Experiments}
    \end{block}
    \vspace{-0.3cm}
    \begin{columns}[T]
        \column{0.5\textwidth}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Graphs: $n \in \{100, 200, 500, 1000, 2000, 5000\}$
            \item Fixed $p=0.05$
            \item Measures runtime and modularity
        \end{itemize}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Fixed $n=500$
            \item Vary $p \in \{0.02, 0.05, 0.10, 0.20, 0.35, 0.50\}$
            \item Shows density effect on quality
        \end{itemize}
        
        \vspace{0.1cm}
        \textbf{3. Quality Evaluation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Planted partition model
            \item $k=4$ communities, $n=200$ per community
            \item Vary $p_{\text{intra}} \in \{0.05, 0.10, 0.15, 0.20, 0.25, 0.30\}$
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Fixed $n=800$, $k=4$, $p_{\text{intra}}=0.2$, $p_{\text{inter}}=0.01$
            \item Vary $\gamma \in \{0.5, 0.75, 1.0, 1.25, 1.5, 2.0\}$
            \item Tests robustness to resolution
        \end{itemize}
        
        \vspace{0.1cm}
        \textbf{5. Algorithm Comparison}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Louvain vs Leiden
            \item Ambiguous graphs ($p_{\text{intra}}=0.15$, $p_{\text{inter}}=0.05$)
            \item Compare quality and speed
        \end{itemize}
        
        \vspace{0.1cm}
        \textbf{Graph Model:}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Planted partition: $k$ communities
            \item $p_{\text{intra}}$: within-community edge prob.
            \item $p_{\text{inter}}$: between-community edge prob.
        \end{itemize}
    \end{columns}
\end{frame}
...
\begin{frame}{Bonus Disclosure}
    \small
    \begin{block}{Bonus Components --- The following should be evaluated for bonus marks:}
        \begin{enumerate}\setlength{\itemsep}{2pt}
            \item \textbf{Community Detection Algorithms:}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Louvain algorithm (modularity optimization)
                \item Leiden algorithm (improved Louvain with quality guarantees)
                \item Modularity metric implementation
            \end{itemize}
            
            \item \textbf{Advanced Centrality:}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Betweenness Centrality (Brandes algorithm)
                \item Harmonic Closeness Centrality
            \end{itemize}
            
            \item \textbf{Recommendation System Enhancements:}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Multi-signal hybrid architecture (Jaccard + Adamic-Adar + Tags)
                \item Noise robustness evaluation
                \item Quality metrics (Precision, Recall, Hit Rate)
            \end{itemize}
        \end{enumerate}
    \end{block}
\end{frame}
